...
export const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  // Compute online/offline summary for currently loaded devices
  useEffect(() => {
    let cancelled = false;
    const run = async () => {
      ...
      try {
        const results = await Promise.all(
          devices.map(async (d: any) => {
            try {
              if (!d.ip || String(d.ip).toLowerCase() === 'n/a') return null;
        ...
        if (cancelled) return;
        const on = results.filter((v) => v === true).length;
        const off = results.filter((v) => v === false).length;
        setOnlineCount(on);
        setOfflineCount(off);
      } finally {
